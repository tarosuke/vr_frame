...
namespace core{
	...
	public:
		SphereMap(const Params& params, const TB::Image& image) :
		...
		static Scenery* New(const TB::Image& image){
			Params params = {
				...
				numOfVertex: sizeof(buffers.vb) / sizeof(TB::VBO::V_UV),
				vertex: (TB::VBO::V_UV*)buffers.vb
			};

			return new SphereMap(params, image);
		};
